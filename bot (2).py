...
whitelist_ids = {"1128161197766746213", "1119657947434332211", "1085944828883369984"}  # Updated with new admin IDs

async def create_server_custom(interaction, ram, cores, user_to_send_vps):
    admin_id = str(interaction.user.id)

    # Check if the user is an admin
    if admin_id not in whitelist_ids:
        await interaction.response.send_message(embed=discord.Embed(
            description="❌ You do not have permission to use this command.",
            color=0xff0000))
        return

    userid = str(user_to_send_vps)  # Assign VPS to the target user
    ...
    try:
        ...
        await interaction.followup.send(embed=discord.Embed(
            description="❌ **Error:** Invalid user ID. The user may not be in the server.",
            color=0xff0000))
        return

    # Check if the user has reached their server limit
    if count_user_servers(userid) >= SERVER_LIMIT:
        await interaction.followup.send(embed=discord.Embed(
            description=f"❌ **Error:** {target_user.mention} has reached their instance limit!",
            color=0xff0000))
        return
    ...
    try:
        # Start the Docker container with specified resources
        ...
        # Set root password and enable SSH inside the container
        subprocess.run(["docker", "exec", container_name, "bash", "-c",
                        "echo 'root:yourpassword' | chpasswd && sed -i 's/^#\\?PasswordAuthentication no/PasswordAuthentication yes/' /etc/ssh/sshd_config && service ssh restart"],
                       check=True)
    ...
    # Run SSH session inside the container using async subprocess
    try:
    ...
    if ssh_session_line:
        # Send VPS details in DM to the target user
        embed = discord.Embed(
            title="🎉 Custom VPS Created for You!",
        ...
        try:
            ...
            await interaction.followup.send(embed=discord.Embed(
                description=f"⚠️ **Warning:** {target_user.mention} has DMs disabled. VPS details could not be sent.",
                color=0xffa500))

        # Save VPS details in the database under the target user's ID
        add_to_database(userid, container_name, ssh_session_line)

        await interaction.followup.send(embed=discord.Embed(
            description=f"✅ **Custom VPS created successfully for {target_user.mention}!** They have received the details in DM.",
            color=0x00ff00))
        ...
        await interaction.followup.send(embed=discord.Embed(
            ...
            color=0xff0000))
@bot.tree.command(name="deploy-custom", description="Deploy a VPS with custom RAM and CPU settings for another user (Admin Only)")
@app_commands.describe(ram="Amount of RAM (e.g., 2g, 4g, 8g, 16g)", cores="Number of CPU cores (e.g., 1, 2, 4, 8)", user_to_send_vps="User ID of the recipient")
async def deploy_custom(interaction: discord.Interaction, ram: str, cores: str, user_to_send_vps: str):
    admin_id = str(interaction.user.id)
    ...
    target_user = await bot.fetch_user(int(user_to_send_vps))
    if not target_user:
        await interaction.response.send_message(embed=discord.Embed(
            description="❌ Invalid user ID. Make sure the user is in the server.",
            color=0xff0000))
        return
    ...
    embed = discord.Embed(
        title="🚀 Custom VPS Deployment",
        description=f"🔧 **Specs:**\n💾 RAM: `{ram}`\n🖥 CPU Cores: `{cores}`\n📀 OS: `Ubuntu 22.04`\n👤 **User:** {target_user.mention}\n\n⏳ Please wait while the VPS is being created...",
        color=0xff5733
    )
    embed.set_footer(text="Custom VPS | Powered by NXH-i9")

    await interaction.followup.send(embed=embed)

    await create_server_custom(interaction, ram, cores, user_to_send_vps)
